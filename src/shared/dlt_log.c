...
bool dlt_is_file_logging_enabled(const DltLoggingMode mode)
{
    if ((mode < DLT_LOG_TO_CONSOLE) || (mode > DLT_LOG_DROPPED)) {
        dlt_vlog(LOG_WARNING, "Wrong parameter for mode: %u\n", mode);
        return false;
    }

    logging_mode = mode;

    return logging_mode == DLT_LOG_TO_FILE;
}
...
{
    if (!dlt_is_file_logging_enabled(mode)) {
